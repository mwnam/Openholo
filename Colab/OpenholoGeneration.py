...
def pointGenerate(Encoding,Precision,pc_mode):
    ...
    if file.exists ():
        if plyfile.exists () :
            subprocess.run(['./OpenholoGeneration -a \"{}\" -c \"{}\" -i \"{}\" -e \"{}\" -m \"{}\" -p \"{}\"'.format("0",PointConfig, ply, enc, pre, mode)], shell=True)
        else:
    ...
    return './Result/PointCloud_'+Encoding+'.bmp'


def depthGenerate(Encoding,Precision,pc_mode):
    file = pathlib.Path("OpenholoGeneration")
    currentPath = os.getcwd()
    ...
    if file.exists () :
        if rgbfile.exists () and depthfile.exists () :
            subprocess.run(['./OpenholoGeneration -a \"{}\" -c \"{}\" -i \"{}\" \"{}\" \"{}\" -e \"{}\" -m \"{}\" -p \"{}\"'.format("1",DepthConfig, './DepthMap', rgb, depth, enc, pre, mode)], shell=True)
        else:
             print("File upload did not complete. Please try again after the file is uploaded.")
    else:
        print("OpenholoGeneration file does not exist, so it does not work normally")
...
def lightGenerate(Encoding,Precision,pc_mode):
    file = pathlib.Path("OpenholoGeneration")
    ...
    if file.exists ():
        if lightdir.exists () :
            subprocess.run(['./OpenholoGeneration -a \"{}\" -c \"{}\" -i \"{}\" -e \"{}\" -m \"{}\" -p \"{}\"'.format("2",LightConfig, light, enc, pre, mode)], shell=True)
        else:
            print("File upload did not complete. Please try again after the file is uploaded.")
    else:
        print("OpenholoGeneration file does not exist, so it does not work normally")
...
def trimeshGenerate(Encoding,Precision,pc_mode):
    file = pathlib.Path("OpenholoGeneration")
    ...
    if file.exists ():
        if trimeshfile.exists () :
            subprocess.run(['./OpenholoGeneration -a \"{}\" -c \"{}\" -i \"{}\" -e \"{}\" -m \"{}\" -p \"{}\"'.format("3",TrimeshConfig, trimesh, enc, pre, mode)], shell=True)
        else:
            print("File upload did not complete. Please try again after the file is uploaded.")
    else:
        print("OpenholoGeneration file does not exist, so it does not work normally")
...
def wrpGenerate(Encoding,Precision,pc_mode):
    file = pathlib.Path("OpenholoGeneration")
    ...
    if file.exists ():
        if wrpplyfile.exists () :
            subprocess.run(['./OpenholoGeneration -a \"{}\" -c \"{}\" -i \"{}\" -e \"{}\" -m \"{}\" -p \"{}\"'.format("4",WrpConfig, wrpply, enc, pre, mode)], shell=True)
        else:
            print("File upload did not complete. Please try again after the file is uploaded.")
    else:
        print("OpenholoGeneration file does not exist, so it does not work normally")
...
with gr.Blocks(theme=gr.themes.Soft(), css=".gradio-container {background-color: #CFE3EA}", title="Openholo Generation") as demo:
    ...
    with gr.Tab(label = "Point Cloud"):
        ...
        with gr.Column():
            ...
            with gr.Row() as row:
                with gr.Column():
                    Encoding = gr.Dropdown(['Phase','Amplitude','Real','Imaginary'], label = "Encoding Method" , info="Choose an encoding!")
            with gr.Row() as row:
                with gr.Column():
                    Precision = gr.Radio(["single", "double"], label="Precision (GPGPU)")
            with gr.Row() as row:
                with gr.Column():
                    pc_mode = gr.Radio(["CPU", "GPU"], label="CPU / GPU")
        ...
        pointGen.click(fn=pointGenerate,inputs=[Encoding,Precision,pc_mode],outputs=pointOutput)
    ...
    with gr.Tab(label = "DepthMap"):
        ...
        with gr.Column():
            ...
            with gr.Row() as row:
                with gr.Column():
                    Propagation = gr.Dropdown(['Angular Spectrum'], label = "Propagation Method" , info="Choose an Propagation!")
            with gr.Row() as row:
                with gr.Column():
                    Encoding = gr.Dropdown(['Phase','Amplitude','Real','Imaginary'], label = "Encoding Method", info="Choose an Encoding!")
            with gr.Row() as row:
                with gr.Column():
                    Precision = gr.Radio(["single", "double"], label="Precision (GPGPU)", info="Choose an Precision!")
            with gr.Row() as row:
                with gr.Column():
                    pc_mode = gr.Radio(["CPU", "GPU"], label="CPU / GPU", info="Choose an mode!")
        ...
        depthGen.click(fn=depthGenerate, inputs=[Encoding,Precision,pc_mode] ,outputs=depthOutput)
    ...
    with gr.Tab(label = "LightField"):
        ...
        lightGen.click(fn=lightGenerate, inputs=[Encoding,Precision,pc_mode] ,outputs=lightOutput)
    ...
    with gr.Tab(label = "Triangle Mesh"):
        ...
        trimeshGen.click(fn=trimeshGenerate,inputs=[Encoding,Precision,pc_mode],outputs=trimeshOutput)
    ...
    with gr.Tab(label = "WRP"):
        ...
        wrpGen.click(fn=wrpGenerate,inputs=[Encoding,Precision,pc_mode],outputs=wrpOutput)
